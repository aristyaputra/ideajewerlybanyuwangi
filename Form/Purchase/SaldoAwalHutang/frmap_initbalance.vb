...
Public Class frmap_initbalance
    ...
    Private Sub detail(ByVal criteria As String, ByVal supplier As String)
        open_conn()
        Dim DT As DataTable
        ...
        Else
            var_chk = 0
        End If
        DT = select_master2("select_ap_initbalance", "invoice_no", criteria, supplier, 1, var_chk, tglawal.Value, tglakhir.Value)
        If DT.Rows.Count > 0 Then
            txt_date.Value = DT.Rows(0).Item("cutoff_date").ToString
            txt_servicenm.Text = DT.Rows(0).Item("invoice_no").ToString
            txt_servicenm.Enabled = False
            cbo_acc.Text = DT.Rows(0).Item("id_supplier").ToString
            cbo_acc.Enabled = False
            lbl_acc_name.Text = DT.Rows(0).Item("supplier_name").ToString
            txt_price.Text = FormatNumber(DT.Rows(0).Item("amount_of"), 0)
    ...
    Private Sub frmap_initbalance_Activated(sender As Object, e As System.EventArgs) Handles Me.Activated
    ...
    Private Sub btn_save2_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_save2.Click
        open_conn()
        If insert = 1 Then
            If getTemplateAkses(username, "MN_AP_INITBALANCE_ADD") <> True Then
        ...
        If edit = 1 Then
            If getTemplateAkses(username, "MN_AP_INITBALANCE_EDIT") <> True Then
                Dim info As AlertInfo = New AlertInfo("Cek Kevaliditasan Data", "Anda tidak memiliki hak akses")
                alertControl_warning.Show(Me, info)
                Exit Sub
            End If
    ...
    Public Sub insert_data()
        open_conn()
        If insert = 1 Then
            Call insert_ap_init(Format(txt_date.Value, "yyyy-MM-dd"), Trim(txt_servicenm.Text), Trim(cbo_acc.Text), Trim(txt_price.Text), _
            ...
            End If
        ElseIf edit = 1 Then
            Call update_ap_init(Format(txt_date.Value, "yyyy-MM-dd"), Trim(txt_servicenm.Text), Trim(cbo_acc.Text), Trim(txt_price.Text), _
    ...
    Private Sub cbo_acc_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles cbo_acc.Click
        Dim FrmAccount As New frm_display_supp
        open_conn()
        frm_display_supp.formsource_apinit = True
    ...
    Private Sub GridList_Customer_DoubleClick(sender As Object, e As System.EventArgs) Handles GridList_Customer.DoubleClick
        ...
        detail(GridList_Customer.GetRowCellValue(GridList_Customer.FocusedRowHandle, "invoice_no"), GridList_Customer.GetRowCellValue(GridList_Customer.FocusedRowHandle, "id_supplier"))
    End Sub
    ...
    Private Sub view_data()
        ...
        Try
            ...
            Else
                var_chk = 0
            End If
            DT = select_master2("select_ap_initbalance", Trim(cbo_search.Text), Trim(txt_search.Text), "", 0, var_chk, tglawal.Value, tglakhir.Value)
            ...
            GridList_Customer.Columns("id_supplier").Caption = "ID Supplier"
            GridList_Customer.Columns("supplier_name").Caption = "Nama Supplier"
    ...
    Private Sub btn_del2_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_del2.Click
        ...
        If edit = 1 Then
            If getTemplateAkses(username, "MN_AP_INITBALANCE_DELETE") <> True Then
                Dim info As AlertInfo = New AlertInfo("Cek Kevaliditasan Data", "Anda tidak memiliki hak akses")
                alertControl_warning.Show(Me, info)
                Exit Sub
            End If
        ...
        If edit = 1 Then
            pesan = MessageBox.Show("Hapus Data?", "Delete", MessageBoxButtons.YesNo)
            If pesan = vbYes Then
                Call delete_ap_init(Format(txt_date.Value, "yyyy-MM-dd"), Trim(txt_servicenm.Text), Trim(cbo_acc.Text), Trim(txt_price.Text), _
