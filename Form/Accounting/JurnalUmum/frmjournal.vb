...
Public Class frmjournal
    ...
    Private Sub frmjournal_Activated(sender As Object, e As System.EventArgs) Handles Me.Activated
    ...
    'Private Sub frmjournal_KeyDown(sender As Object, e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
    ...
    Private Sub frmjournal_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Call insert_no_trans("JOURNAL", Month(txt_date.Value), Year(txt_date.Value))
        Call select_control_no("JOURNAL", "TRANS")
    ...
    Private Sub view_data(ByVal pageIndex As Integer, ByVal pageLoad As Integer)
        ...
        Try
            ...
            Else
                var_date_filter = 0
            End If
            DT = select_journal(Trim(cbo_search.Text), Trim(txt_search.Text), 0, var_date_filter, tglawal.Value, tglakhir.Value, pageIndex, pageLoad)
    ...
    Private Sub insert_data()
        ...
        Dim i As Integer
        If insert = 1 Then
            Call insert_journal(Trim(txt_nojournal.Text), txt_date.Value, Trim(txt_comment.Text), username, Format(server_datetime(), "yyyy-MM-dd"), _
            ...
            If param_sukses = True Then
                ...
                update_no_trans(txt_date.Value, "JOURNAL")
                clean()
            Else
            ...
            End If
        ElseIf edit = 1 Then
            Call update_journal(Trim(txt_nojournal.Text), txt_date.Value, Trim(txt_comment.Text), username, Format(server_datetime(), "yyyy-MM-dd"), _
    ...
    Private Sub btn_save2_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_save2.Click
        ...
        If insert = 1 Then
            If getTemplateAkses(username, "MN_JOURNAL_ADD") <> True Then
        ...
        If edit = 1 Then
            If getTemplateAkses(username, "MN_JOURNAL_EDIT") <> True Then
                Dim info As AlertInfo = New AlertInfo("Cek Kevaliditasan Data Hak Akses", "Anda tidak memiliki hak akses!")
                alertControl_warning.Show(Me, info)
                Exit Sub
            End If
        ...
        If trial = True Then
            If get_count_data("trn_journal_head", "journal_no") > row_trial Then
    ...
    Private Sub btn_del2_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_del2.Click
        ...
        Dim var_nominal As Double
        If edit = 1 Then
            If getTemplateAkses(username, "MN_JOURNAL_DELETE") <> True Then
                Dim info As AlertInfo = New AlertInfo("Cek Kevaliditasan Data Hak Akses", "Anda tidak memiliki hak akses!")
                alertControl_warning.Show(Me, info)
                Exit Sub
            End If
        ...
        If edit = 1 Then
            ...
            If pesan = vbYes Then
                Call delete_journal(Trim(txt_nojournal.Text), txt_date.Value, Trim(txt_comment.Text), username, Format(server_datetime(), "yyyy-MM-dd"), _
                                username, Format(server_datetime(), "yyyy-MM-dd"), "", "", 0, 0, (txt_Tdebit.Text), cbo_curr.Text, txt_kurs.Text)
                If param_sukses = True Then
    ...
    Private Sub detail(ByVal criteria As String)
        ...
        Else
            var_date_filter = 0
        End If
        DT = select_journal("journal_no", criteria, 1, var_date_filter, tglawal.Value, tglakhir.Value, 0, pageLoad)
        rows = DT.Rows.Count - 1
        If DT.Rows.Count > 0 Then
    ...
    Private Sub cbo_noakun_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        open_conn()
        '.Text = cbo_noakun.SelectedItem.Col2
    End Sub
    ...
    Private Sub GroupControl7_Paint(sender As System.Object, e As System.Windows.Forms.PaintEventArgs) Handles GroupControl7.Paint
