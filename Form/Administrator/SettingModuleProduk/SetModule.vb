...
Public Class SetModule
    ...
    Private Sub view_privilege_user()
        ...
        For Each childkey1 As TreeNode In TreeUser.Nodes
            If CompareTemplateHakAkses(childkey1.Name) > 0 Then
            ...
            For Each childkey2 As TreeNode In childkey1.Nodes
                If CompareTemplateHakAkses(childkey2.Name) > 0 Then
                ...
                For Each childkey3 As TreeNode In childkey2.Nodes
                    If CompareTemplateHakAkses(childkey3.Name) > 0 Then
                    ...
                    For Each childkey4 As TreeNode In childkey3.Nodes
                        If CompareTemplateHakAkses(childkey4.Name) > 0 Then
    ...
    Private Sub view_privilege()
        ...
        With TreeUser
            ...
            Do While a <= DT.Rows.Count - 1
                ...
                DTChild = getChildKeyDef(DT.Rows(a).Item("childkey"))
                If DTChild.Rows.Count > 0 Then
                    ...
                    Do While b <= DTChild.Rows.Count - 1
                        ...
                        DTChild2 = getChildKeyDef(DTChild.Rows(b).Item("childkey"))
                        If DTChild2.Rows.Count > 0 Then
                            ...
                            Do While c <= DTChild2.Rows.Count - 1
                                ...
                                DTChild3 = getChildKeyDef(DTChild2.Rows(c).Item("childkey"))
    ...
    Private Sub insert_hakakses()
        ...
        Try
            ...
            SplashScreenManager.Default.SetWaitFormDescription("Loading . . .")
            Dim i As Integer
            Call delete_templateakses()
            Call setautoinc_templateakses()
            'For i = 0 To TreeUser.Nodes.Count - 1

            ' Call setautoinc_templateakses()
            For Each childkey1 As TreeNode In TreeUser.Nodes
                If TreeUser.Nodes.Item(TreeUser.Nodes.IndexOfKey(childkey1.Name)).Checked = True Then
                    parentkey = getParentKey_FromCK_def(childkey1.Name)
                    ' If cek_templateakses(parentkey, childkey1.Name) = 0 Then
                    Call insert_templateakses(parentkey, childkey1.Name, TreeUser.Nodes.Item(TreeUser.Nodes.IndexOfKey(childkey1.Name)).Text, 0)
                    'End If
                End If
                For Each childkey2 As TreeNode In childkey1.Nodes
                    If childkey1.Nodes.Item(childkey1.Nodes.IndexOfKey(childkey2.Name)).Checked = True Then
                        parentkey = getParentKey_FromCK_def(childkey2.Name)
                        '       If cek_templateakses(parentkey, childkey2.Name) = 0 Then
                        Call insert_templateakses(parentkey, childkey2.Name, childkey1.Nodes.Item(childkey1.Nodes.IndexOfKey(childkey2.Name)).Text, 1)
                        'End If
                    End If
                    For Each childkey3 As TreeNode In childkey2.Nodes
                        If childkey2.Nodes.Item(childkey2.Nodes.IndexOfKey(childkey3.Name)).Checked = True Then
                            parentkey = getParentKey_FromCK_def(childkey3.Name)
                            '       If cek_templateakses(parentkey, childkey3.Name) = 0 Then
                            Call insert_templateakses(parentkey, childkey3.Name, childkey2.Nodes.Item(childkey2.Nodes.IndexOfKey(childkey3.Name)).Text, 2)
                            'End If
                        End If
                        'For Each childkey4 As TreeNode In childkey3.Nodes
    ...
    Private Sub fillComboBox()
        Dim DTTemplateHakAkses As DataTable
        DTTemplateHakAkses = getTemplateHakAksesDef_Parent()
        cbo_parentkey.Properties.DataSource = DTTemplateHakAkses
        cbo_parentkey.Properties.DisplayMember = "childkey"
        cbo_parentkey.Properties.ValueMember = "childkey"
        cbo_parentkey.Properties.PopulateViewColumns()
        cbo_parentkey.Properties.BestFitMode = DevExpress.XtraEditors.Controls.BestFitMode.BestFitResizePopup
        cbo_parentkey.Properties.View.OptionsView.ShowAutoFilterRow = True
        cbo_parentkey.Properties.View.Columns("childkey").Caption = "Child Key"
    End Sub
    ...
    Private Sub SimpleButton6_Click(sender As System.Object, e As System.EventArgs) Handles SimpleButton6.Click
        Dim ismenu As Integer
        ismenu = getIsMenuKeyDef(Trim(cbo_parentkey.EditValue)) + 1
        Try
            Call insertTemplateHakAksesDef(cbo_parentkey.EditValue, Trim(txt_childkey.Text), txt_notes.Text, ismenu)
            If param_sukses = True Then
                Dim info As AlertInfo = New AlertInfo(msgtitle_save_success, msgbox_save_success)
                alertControl_success.Show(Me, info)
            End If
