...
Public Class frmassemblyitem
    ...
    Private Sub frmreceiptmoney_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Call select_control_no("ASSEMBLY PRODUCT", "MASTER")
    ...
    Private Sub GridList_Customer_DoubleClick(sender As Object, e As System.EventArgs) Handles GridList_Customer.DoubleClick
        ...
        detail(GridList_Customer.GetRowCellValue(GridList_Customer.FocusedRowHandle, "no_assembly"))
    End Sub
    ...
    Private Sub insert_data()
        ...
        Dim i As Integer
        If insert = 1 Then
            Call insert_assembly(txt_bom_no.Text, cbo_item.EditValue, txt_comment.Text, txt_qty.Text, cbo_unit_produksi.EditValue, Format(txt_date.Value, "yyyy-MM-dd"), _
                                        username, Format(server_datetime(), "yyyy-MM-dd hh:mm:ss"), Format(server_datetime(), "yyyy-MM-dd hh:mm:ss"), username, "", 0, "", _
                                        0, "", 0, "", 0, _
                                        0, _
                                        "", 0, _
                                        0, 0, _
                                       "INSERT", Replace(txt_total_konversi.Text, ",", ""), Replace(txt_total.Text, ",", ""))
            For i = 0 To DataGridView1.Rows.Count - 1
            ...
            If param_sukses = True Then
                ...
                'pesan = MsgBox("Print Bukti Perakitan?", MsgBoxStyle.Question + MsgBoxStyle.YesNo, "Konfirmasi")
                'If pesan = vbYes Then
        ...
        ElseIf edit = 1 Then
            ...
            Call insert_assembly(txt_bom_no.Text, cbo_item.EditValue, txt_comment.Text, txt_qty.Text, cbo_unit_produksi.EditValue, Format(txt_date.Value, "yyyy-MM-dd"), _
                                         ...
                                         "", 0, _
                                         0, 0, _
                                        "UPDATE", Replace(txt_total_konversi.Text, ",", ""), Replace(txt_total.Text, ",", ""))
            For i = 0 To DataGridView1.Rows.Count - 1
                If DataGridView1.Item(1, i).Value <> "" Then
                    Call update_assembly(txt_bom_no.Text, cbo_item.EditValue, txt_comment.Text, txt_qty.Text, cbo_unit_produksi.EditValue, Format(txt_date.Value, "yyyy-MM-dd"), _
                                        username, Format(server_datetime(), "yyyy-MM-dd hh:mm:ss"), Format(server_datetime(), "yyyy-MM-dd hh:mm:ss"), username, _
                                        DataGridView1.Item(1, i).Value, _
                                        Replace(DataGridView1.Item(4, i).Value, ",", ""), _
                                        DataGridView1.Item(5, i).Value, _
    ...
    Private Sub view_data()
        ...
        Try
            ...
            ElseIf chk_date.Checked = False Then
                date_filter = 0
            End If
            DT = select_assembly(Trim(txt_search.Text), 0, date_filter, Format(tglawal.Value, "yyyy-MM-dd"), Format(tglakhir.Value, "yyyy-MM-dd"))
            GridControl.DataSource = DT
            GridList_Customer.Columns("no_assembly").Caption = "No Perakitan"
            GridList_Customer.Columns("id_item").Caption = "Kode Produk"
            GridList_Customer.Columns("item_name").Caption = "Deskripsi Produk"
            GridList_Customer.Columns("qty_item_production").Caption = "Jumlah Perakitan"
    ...
    Private Sub detail(ByVal criteria As String)
        ...
        DT = select_assembly(criteria, 1, date_filter, Format(tglawal.Value, "yyyy-MM-dd"), Format(tglakhir.Value, "yyyy-MM-dd"))
        rows = DT.Rows.Count - 1
        If DT.Rows.Count > 0 Then
            cbo_item.EditValue = DT.Rows(0).Item("id_item")
            txt_bom_no.Text = DT.Rows(0).Item("no_assembly")
        ...
        End If
        DTDetail1 = select_assembly(criteria, 2, date_filter, Format(tglawal.Value, "yyyy-MM-dd"), Format(tglakhir.Value, "yyyy-MM-dd"))
        ...
        DTDetail3 = select_assembly(criteria, 4, date_filter, Format(tglawal.Value, "yyyy-MM-dd"), Format(tglakhir.Value, "yyyy-MM-dd"))
        ...
        Dim subtotal_material, subtotal_konveri As Double
        rows = DataGridView4.Rows.Count - 1
        For i = 0 To rows
            subtotal_konveri = subtotal_konveri + Replace(DataGridView4.Item(6, i).Value, ",", "")
        Next

        hitung_nominal_konversi()
        rows = DataGridView1.Rows.Count - 1
        For i = 0 To rows
            subtotal_material = subtotal_material + Replace(DataGridView1.Item(7, i).Value, ",", "")
        Next

        txt_total_konversi.Text = FormatNumber(subtotal_konveri, 0)
        txt_total.Text = FormatNumber(subtotal_material, 0)
    End Sub
    ...
    Private Sub btn_save2_Click(sender As System.Object, e As System.EventArgs) Handles btn_save2.Click
        open_conn()
        If insert = 1 Then
            If getTemplateAkses(username, "MN_ITEM_ASSEMBLY_ADD") <> True Then
        ...
        If edit = 1 Then
            If getTemplateAkses(username, "MN_ITEM_ASSEMBLY_EDIT") <> True Then
                Dim info As AlertInfo = New AlertInfo("Cek Kevaliditasan Data", "Anda tidak memiliki hak akses")
                alertControl_warning.Show(Me, info)
                Exit Sub
            End If
    ...
    Private Sub btn_del2_Click(sender As System.Object, e As System.EventArgs) Handles btn_del2.Click
        open_conn()
        If edit = 1 Then
            If getTemplateAkses(username, "MN_ITEM_ASSEMBLY_DELETE") <> True Then
                Dim info As AlertInfo = New AlertInfo("Cek Kevaliditasan Data", "Anda tidak memiliki hak akses")
                alertControl_warning.Show(Me, info)
                Exit Sub
            End If
            ...
            If pesan = vbYes Then
                Call delete_assembly(txt_bom_no.Text, cbo_item.EditValue, txt_comment.Text, txt_qty.Text, cbo_unit_produksi.EditValue, Format(txt_date.Value, "yyyy-MM-dd"), _
                                         username, Format(server_datetime(), "yyyy-MM-dd hh:mm:ss"), Format(server_datetime(), "yyyy-MM-dd hh:mm:ss"), username, "", 0, "", _
                                         0, "", 0, "", 0, _
                                         0, _
                                         "", 0, _
                                         0, 0, _
                                        "DELETE", Replace(txt_total_konversi.Text, ",", ""), Replace(txt_total.Text, ",", ""))
                If param_sukses = True Then
